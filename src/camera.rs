use std::fs::File;
use std::io::Write;
use crate::{vector3::{Vector3, Point3, Color}, ray3::Ray3, util::{degree_to_radians, self, write_color, random_in_unit_disk, random_double_range, random_double}, hittable_list::HittableList, hittable::Hittable};
...
impl Camera {
    ...
    pub fn render(&self, file: &mut File, world: &HittableList, sample_count: u32) {
        let max_depth = 50;

        let image_width = self.image_width as u32;
        let image_height = self.image_height as u32;
    ...
    pub fn get_ray(&self, u: f32, v: f32) -> Ray3 {
        ...
        let time = random_double();
        Ray3::new(origin, pixel_sample - origin, time)
    }
