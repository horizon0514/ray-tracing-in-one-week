...
impl Material for Lambertian {
    fn scatter(&self, ray: &Ray3, rec: &HitRecord) -> Option<(Color, Ray3)> {
        ...
        let scattered = Ray3::new(rec.point, scatter_direction, ray.time);
...
impl Material for Metal {
    fn scatter(&self, ray: &Ray3, rec: &HitRecord) -> Option<(Color, Ray3)> {
        ...
        let scattered = Ray3::new(rec.point, reflected + random_unit_vector() * self.fuzz, ray.time);
        let attenuation = self.albedo;
...
impl Material for  Dielectric {
    fn scatter(&self, ray: &Ray3, rec: &HitRecord) -> Option<(Color, Ray3)> {
        ...
        Some((attenuation, Ray3::new(rec.point, direction, ray.time)))
    }
}